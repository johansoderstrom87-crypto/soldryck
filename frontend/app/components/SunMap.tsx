...
// Optional metro network data (tracks + platforms) — generated by pipeline/09_fetch_metro.py
let metroNetwork: { METRO_TRACKS: { color: "red" | "green" | "blue"; coords: [number, number][] }[]; METRO_PLATFORMS: { coords: [number, number][] }[] } | null = null;
try {
...
export default function SunMap({ hour: hourProp, date, filter, typeFilter, sunRange, weather, onFeedback, showShadows, showMetro, focusVenueId, onFocusHandled, metroStation }: SunMapProps) {
  ...
  useEffect(() => {
    ...
    // Tracks first so platforms render on top
    ...
    // Platforms: dark outline + bright inner stroke so they pop against any line color
