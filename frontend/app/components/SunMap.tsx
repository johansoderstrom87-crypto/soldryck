...
let metroNetwork: {
  ...
  METRO_STATIONS: { lat: number; lng: number; name: string; endpoints: [[number, number], [number, number]] }[];
...
export default function SunMap({ hour: hourProp, date, filter, typeFilter, sunRange, weather, onFeedback, showShadows, showMetro, focusVenueId, onFocusHandled, metroStation }: SunMapProps) {
  ...
  useEffect(() => {
    ...
    // Station pills — rendered as polylines with geographic length so they
    // scale with zoom (just like a real ~140m platform on a real map). Two
    // stacked polylines: dark border + white inner = clean pill look.
    for (const station of metroNetwork.METRO_STATIONS) {
      L.polyline(station.endpoints, {
        color: "#1e293b",
        weight: 6,
        opacity: 0.95,
        lineCap: "round",
