...
// Optional metro network data (tracks + platforms + entrances) — generated by pipeline/09_fetch_metro.py
let metroNetwork: {
  METRO_TRACKS: { color: "red" | "green" | "blue"; coords: [number, number][] }[];
  METRO_PLATFORMS: { coords: [number, number][] }[];
...
export default function SunMap({ hour: hourProp, date, filter, typeFilter, sunRange, weather, onFeedback, showShadows, showMetro, focusVenueId, onFocusHandled, metroStation }: SunMapProps) {
  ...
  useEffect(() => {
    ...
    // Order matters within overlayPane — later additions render on top.
    // Tracks → connectors (dashed) → platforms (slim strecks) → entrance dots.
    ...
    // Connector lines from each platform endpoint to its entrance — dashed, neutral
    ...
    // Platforms: single slim streck along the platform's long axis
    for (const plat of metroNetwork.METRO_PLATFORMS) {
      L.polyline(plat.coords, {
        color: "#0f172a",
        weight: 3,
        opacity: 0.85,
        lineCap: "round",
    ...
    // Entrance markers: small filled dots on top of everything
    for (const ent of metroNetwork.METRO_ENTRANCES) {
      const entranceIcon = L.divIcon({
        className: "metro-entrance",
        html: `<div class="metro-entrance-dot"${ent.name ? ` title="${ent.name.replace(/"/g, "&quot;")}"` : ""}></div>`,
        iconSize: [10, 10],
        iconAnchor: [5, 5],
      });
