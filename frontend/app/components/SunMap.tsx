...
let metroNetwork: {
  ...
  METRO_STATIONS: { lat: number; lng: number; name: string; angle: number; lenM: number }[];
...
export default function SunMap({ hour: hourProp, date, filter, typeFilter, sunRange, weather, onFeedback, showShadows, showMetro, focusVenueId, onFocusHandled, metroStation }: SunMapProps) {
  ...
  useEffect(() => {
    ...
    // Render order (later = on top): tracks → connectors → station pills → entrance dots.
    ...
    // Premium dotted connector — fine round dots in slate-500 instead of harsh dashes
    ...
    // Station pills — small white-filled rounded rect aligned with platform axis.
    // The CSS rotation (pre-computed in pipeline) makes the pill point along the platform.
    for (const station of metroNetwork.METRO_STATIONS) {
      const titleAttr = station.name ? ` title="${station.name.replace(/"/g, "&quot;")}"` : "";
      const stationIcon = L.divIcon({
        className: "metro-station",
        html: `<div class="metro-station-pill" style="transform:rotate(${station.angle}deg)"${titleAttr}></div>`,
        iconSize: [20, 20],
        iconAnchor: [10, 10],
      });
      L.marker([station.lat, station.lng], { icon: stationIcon, interactive: !!station.name, zIndexOffset: -30 })
        .addTo(layer);
    }

    // Entrance dots — smaller, dark slate, sit at the actual exit location
    for (const ent of metroNetwork.METRO_ENTRANCES) {
